...
class ExpenseManager {
	...
	/*
	 * addTransaction method is responsible for doing transactions (like expenses,
	 * income, transfer)
	 * 
	 * Expense will be when a from is a user and to is not a user because that will
	 * be for food, Entertainment, etc.
	 * Income will be when from Sponsor or fees so in this case from will be null
	 * and to will be a user.
	...
	// This function is responsible for getting the net balance of Avishkar like by
	// adding funds and subtracting expense.
	...
	// This function is responsible to get Expense in a particular category from a
	// start Time to an end Time
	...
	// This funcion is responsible to get Expense for a particular Department from a
	// start Time to an end Time
	double getExpenseOfDepartment(DateTime start, DateTime end, Department department) {
		double expense = 0;
		for (Transaction transaction : transactions) {
			if (dateCmp(start, transaction.datetime) && dateCmp(transaction.datetime, end)) {
				if (transaction.from != null && transaction.from.user_position != Position.values()[0]
						&& transaction.from.user_department == department && transaction.to == null)
	...
	double getIncomeOfDepartment(DateTime start, DateTime end, Department department) {
		...
		for (Transaction transaction : transactions) {
			if (dateCmp(start, transaction.datetime) && dateCmp(transaction.datetime, end)) {
				if (transaction.to != null && transaction.to.user_position == Position.values()[1]
						&& transaction.to.user_department == department && transaction.from == null)
	...
	double getCashflowOfDepartment(Department department, DateTime datetime) {
		return getIncomeOfDepartment(new DateTime(2022, 11, 1, 0, 0), datetime, department)
				- getExpenseOfDepartment(new DateTime(2022, 11, 1, 0, 0), datetime, department);
	}
	...
	double getMoneyTransferredBetweenInterDepartments() {
		...
		for (Transaction transaction : transactions) {
			if (transaction.from != null && transaction.to != null
					&& transaction.from.user_position != Position.values()[0]
					&& transaction.to.user_position != Position.values()[0]
					&& transaction.from.user_department != transaction.to.user_department) {
	...
	List<Department> getListOfDeptsInAscendOrderOfIncomeTOExpenseRatio() {
		...
		for (Transaction transaction : transactions) {
			if (transaction.from == null && transaction.to != null
					&& transaction.to.user_position != Position.values()[0]) {
				incomeMap.put(transaction.to.user_department,
						incomeMap.get(transaction.to.user_department) + transaction.amount);
			}
			if (transaction.to == null && transaction.from != null
					&& transaction.from.user_position != Position.values()[0]) {
				expenseMap.put(transaction.from.user_department,
						expenseMap.get(transaction.from.user_department) + transaction.amount);
			}
		}
