...
{ 
   ...
   while(n--)
   {
     ...
     {
      int dd = names.size();
      string nn = names[rand()%dd];
      // 1w
      if(ddone.find(nn)!=ddone.end())
      {
        n++; continue;
      }
      cout<<0<<" "<<nn<<" "<<rand()%2<<" "<<rand()%9<<endl;
      ddone.insert(nn);
      fund[nn] = 0;
     }
     else if(curr == 1)
      ...
      if(nn == ny)
      {
       ...
       if(!_from)
       {
        ...
        if(!_to)
        {
          ...
          if(fund[nn] == 0){n++;continue;}
          amt = rand()%fund[nn];
          fund[nn]-=amt;
        }
        if(!_from and !_to){n++;continue;}
        if(_from*_to)
        {
          if(nn.empty()+ny.empty())
          {
            n++; continue;
          }
          addMin(day, hour, minute, 10+rand()%(190));
          cout<<1<<" "<<nn<<" "<<ny<<" "<<amt<<" "<<rand()%9<<" "<<2022<<" "<<11<<" "<<day<<" "<<hour<<" "<<minute<<endl;
          fund[ny]+=amt;
        }
        else
        {
          addMin(day, hour, minute, 10+rand()%(190));
          if(!_from)
          cout<<1<<" "<<"null"<<" "<<nn<<" "<<amt<<" "<<rand()%9<<" "<<2022<<" "<<day<<" "<<hour<<" "<<minute<<endl;
        else
        {
          addMin(day, hour, minute, 10+rand()%(190));
          cout<<1<<" "<<nn<<" "<<"null"<<" "<<amt<<" "<<rand()%9<<" "<<2022<<" "<<day<<" "<<hour<<" "<<minute<<endl;
        }
        }
     }
     else if(curr == 2)
     ...
     {
      int day1 = rand()%31;
      int day2 = day1+rand()%(30-day1+1);
      int category = rand()%9;
      // cout<<4<<" "<<day1<<" "<<day2<<endl;
      cout<<4<<" "<<2022<<" "<<11<<" "<<day1<<" "<<(rand()%12)<<" "<<(rand()%60)<<" "<<2022<<" "<<11<<" "<<day2<<" "<<(rand()%12)<<" "<<(rand()%60)<<" "<<category<<endl;
     }
     else if(curr == 5)
     {
      int day1 = rand()%31;
      int day2 = day1+rand()%(30-day1+1);
      int category = rand()%9;
      cout<<5<<" "<<2022<<" "<<11<<" "<<day1<<" "<<(rand()%12)<<" "<<(rand()%60)<<" "<<2022<<" "<<11<<" "<<day2<<" "<<(rand()%12)<<" "<<(rand()%60)<<" "<<category<<endl; 
     }
     else if(curr == 6)
     {
      int day1 = rand()%30;
      int day2 = day1+rand()%(30-day1+1);
      int category = rand()%9;
      cout<<6<<" "<<2022<<" "<<11<<" "<<day1<<" "<<(rand()%12)<<" "<<(rand()%60)<<" "<<category<<endl; 
     }
      else
        cout<<curr<<endl;
   }
